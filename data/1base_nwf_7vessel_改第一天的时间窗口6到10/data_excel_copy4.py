...
path="C:/Users/liuzx/Desktop/智能调度决策——自动化所/自动化所——调度/代码新/数据导入4.xlsx"
...
class_vessels = {0: '#FF0000', 1: '#008000', 2: '#0000FF',3:'#00FFFF',4:'#00FF00',5:'#FF4040',6:'#B99272'}  # 船的类型不同画出来的线颜色不同
...
def main():
    ...
    for i in range(num_wf):
        dataframe = pd.DataFrame({'day': range(data['period']), 'WF%dV1'%(i+1): routes[0][i], 'WF%dV2'%(i+1): routes[1][i], 'WF%dV3'%(i+1): routes[2][i],'WF%dV4'%(i+1): routes[3][i],\
                                  'WF%dV5'%(i+1): routes[4][i],'WF%dV6'%(i+1): routes[5][i],'WF%dV7'%(i+1): routes[6][i]})
    ...
    # #导出坐标
    # coordinate_list=[]
    # coordinatepath="coordinate_result.xls"
    # result = open( coordinatepath, 'w', encoding='gbk')
    # #此处解决单码头的情况
    # base0=(2245000,434600) #虚拟码头
    # count_wturbine=[0,36] #不同风电场的风机所在位置
    # for iday in num_day:
    #     coordinate_list.append([])
    #     for ivessels in range(num_vessel):
    #         for iwf in range(num_wf):
    #             iplan_path = routes[ivessels][iwf][iday]
    #             # print('iplan_path:', iplan_path)
    #             if iplan_path == []:
    #                 continue
    #             if len(iplan_path)==2: #说明风机只访问一个节点
    #                 coordinate_list[iday].append([ivessels,iwf,base0,data['wind_farm']['coordinate'][iwf][iplan_path[0]-count_wturbine[iwf]],base0])
    #             if len(iplan_path)> 2: #区分船是否要在风机处停,此处后续还要改，有可能一个船一次访问>2的风机
    #                 if iplan_path[0]==iplan_path[1]: #意味着船在该风机编号维修时需要停留
    #                     coordinate_list[iday].append(
    #                         [ivessels, iwf, base0, data['wind_farm']['coordinate'][iwf][iplan_path[0]-count_wturbine[iwf]],data['wind_farm'] \
    #                             ['coordinate'][iwf][iplan_path[2]-count_wturbine[iwf]], base0])
    #                 else:
    #                     coordinate_list[iday].append(
    #                         [ivessels, iwf, base0, data['wind_farm']['coordinate'][iwf][iplan_path[0]-count_wturbine[iwf]],
    #                          data['wind_farm']['coordinate'][iwf][iplan_path[1]-count_wturbine[iwf]],\
    #                                                               data['wind_farm']['coordinate'][iwf][iplan_path[0]-count_wturbine[iwf]],base0])
    #
    #     result.write('day%d'% iday)
    #     result.write("\n")
    #     for i in range(len(coordinate_list[iday])):
    #         for j in range(len(coordinate_list[iday][i])):
    #             result.write(str(coordinate_list[iday][i][j]))
    #             result.write('\t')
    #         result.write("\n")
    # result.close()
