...
path="C:/Users/liuzx/Desktop/智能调度决策——自动化所/自动化所——调度/代码新/数据导入1.xlsx"
...
def create_data_model():
    ...
    data['wind_farm'] = {}
    count_wind_farm=1  #需要维修的风电场个数
    count_wind_turbine=[8,0] #每个风电场需要维修的风机个数
    count_wind_turbine_sum=[55,0]# 每个风电场所有的风机个数
    ...
    for i in range(count_wind_farm):
        ...
        for j in range(count_wind_turbine_sum[i]):
            ...
            turbine_x = sheet1.cell_value(j + 1 + count_turbine[i], getColumnIndex(sheet1, '国测局风机坐标X'))
            turbine_y = sheet1.cell_value(j + 1 + count_turbine[i], getColumnIndex(sheet1, '国测局风机坐标Y'))
            data['wind_farm']['gcjcoordinate'][i].append((turbine_x, turbine_y))
...
def vision_plot(num_day, num_vessel, num_wf,plan_path):
    # 打开交互模式
    plt.ion()
    # base = (2396689, 414609) #实际港口
    base=(2400000,448000) #虚拟港口
    # plt.plot(X, Y,'o')
...
def main():
    ...
    # # 此处解决单码头的情况
    # base0 = (2396689, 414609)  # 虚拟码头
    # count_wturbine = [0, 55]  # 不同风电场的风机所在位置
    # for iday in num_day:
    ...
    #                 coordinate_list[iday].append([ivessels, iwf, base0, data['wind_farm']['gcjcoordinate'][iwf][
    #                     iplan_path[0] - count_wturbine[iwf]], base0])
    ...
    #                         [ivessels, iwf, base0,
    #                          data['wind_farm']['gcjcoordinate'][iwf][iplan_path[0] - count_wturbine[iwf]],
    #                          data['wind_farm'] \
    #                              ['gcjcoordinate'][iwf][iplan_path[2] - count_wturbine[iwf]], base0])
    #                 else:
    #                     coordinate_list[iday].append(
    #                         [ivessels, iwf, base0,
    #                          data['wind_farm']['gcjcoordinate'][iwf][iplan_path[0] - count_wturbine[iwf]],
    #                          data['wind_farm']['gcjcoordinate'][iwf][iplan_path[1] - count_wturbine[iwf]], \
    #                          data['wind_farm']['gcjcoordinate'][iwf][iplan_path[0] - count_wturbine[iwf]], base0])
    #
